...
func TestProcessCSSModules(t *testing.T) {
	for i := range testCasesCSSModules {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			css, scopedClasses, err := ProcessCSSModules(strings.NewReader(tc.payload))
			if err != nil {
...
func TestCSSModulesParser_ParseTo(t *testing.T) {
	for i := range testCasesCSSModules {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			buf := getBuffer()
			scopedClasses, err := NewCSSModulesParser(strings.NewReader(tc.payload)).ParseTo(buf)
			if err != nil {
				if err.Error() != tc.expectedError {
					t.Errorf("unexpected error value: expected %q got %q", tc.expectedError, err.Error())
					return
			...
			if tc.expectedCSSModules.canMatch {
				if !bytes.Equal(tc.expectedCSSModules.value, buf.Bytes()) {
					t.Errorf("unexpected css slice of bytes value: expected\n%q\ngot\n%q", tc.expectedCSSModules.value, buf.Bytes())
					return
				}
			}
			for i := range tc.expectedScopedClasses {
